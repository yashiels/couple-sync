import { handleRedeemInvite } from '../redeemInvite';
import { InviteDoc } from '../lib/types';

const FUTURE_EXPIRY = Date.now() + 24 * 60 * 60 * 1000; // 24 hours from now
const PAST_EXPIRY = Date.now() - 1000; // 1 second ago

const makeValidInvite = (overrides: Partial<InviteDoc> = {}): InviteDoc => ({
  code: 'ABC123',
  createdByUid: 'userA',
...
const makeDeps = (overrides: Record<string, unknown> = {}) => ({
  getInvite: jest.fn().mockResolvedValue(makeValidInvite()),
  createCouple: jest.fn().mockResolvedValue('couple-id-1'),
  acceptInvite: jest.fn().mockResolvedValue(undefined),
  linkUserToCouple: jest.fn().mockResolvedValue(undefined),
  ...overrides,
});
...
describe('handleRedeemInvite', () => {
  test('returns coupleId on successful redemption', async () => {
    const deps = makeDeps();
    const result = await handleRedeemInvite('userB', 'ABC123', deps);
  ...
  test('creates couple with both user UIDs', async () => {
    const deps = makeDeps();
    await handleRedeemInvite('userB', 'ABC123', deps);

    expect(deps.createCouple).toHaveBeenCalledWith(
  ...
  test('throws FAILED_PRECONDITION when invite is already accepted', async () => {
    const deps = makeDeps({
      getInvite: jest.fn().mockResolvedValue(
        makeValidInvite({ status: 'accepted' })
      ),
    });
  ...
  test('throws FAILED_PRECONDITION when invite is marked expired', async () => {
    const deps = makeDeps({
      getInvite: jest.fn().mockResolvedValue(
        makeValidInvite({ status: 'expired' })
      ),
    });
  ...
  test('throws DEADLINE_EXCEEDED when invite expiry timestamp is in the past', async () => {
    const deps = makeDeps({
      getInvite: jest.fn().mockResolvedValue(
        makeValidInvite({ expiresAt: PAST_EXPIRY })
      ),
    });
  ...
  test('throws INVALID_ARGUMENT when user tries to redeem their own invite', async () => {
    ...
    await expect(handleRedeemInvite('userA', 'ABC123', deps))
