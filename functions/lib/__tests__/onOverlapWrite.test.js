...
const onOverlapWrite_1 = require("../onOverlapWrite");
const makeCouple = () => ({
    userAUid: 'userA',
    userBUid: 'userB',
    status: 'active',
...
const makeWindow = () => ({
    startUtc: Date.now() + 3600000,
    endUtc: Date.now() + 7200000,
    durationMinutes: 60,
    score: 15,
    reasonableBoth: true,
});
const makeDeps = (overrides = {}) => (Object.assign({ getCouple: jest.fn().mockResolvedValue(makeCouple()), getFcmTokens: jest.fn()
        .mockResolvedValueOnce(['token-a1', 'token-a2'])
        .mockResolvedValueOnce(['token-b1']), sendNotification: jest.fn().mockResolvedValue([]), updateFcmTokens: jest.fn().mockResolvedValue(undefined) }, overrides));
describe('handleOnOverlapWrite', () => {
    test('sends notifications to both partners', async () => {
        const deps = makeDeps();
        await (0, onOverlapWrite_1.handleOnOverlapWrite)('couple1', [makeWindow()], deps);
    ...
    test('does nothing when no overlap windows', async () => {
        const deps = makeDeps();
        await (0, onOverlapWrite_1.handleOnOverlapWrite)('couple1', [], deps);
    ...
    test('sends notification with title and body', async () => {
        const deps = makeDeps();
        await (0, onOverlapWrite_1.handleOnOverlapWrite)('couple1', [makeWindow()], deps);
        expect(deps.sendNotification).toHaveBeenCalledWith(expect.any(Array), expect.objectContaining({
            title: expect.any(String),
            body: expect.any(String),
        }));
    });
});
//# sourceMappingURL=onOverlapWrite.test.js.map