...
**Android and iOS are both build targets. Google is the only sign-in method.** Apple Sign-In is not
used — sign-in is Google-only on both platforms. (iOS ships via TestFlight, built in CI on macOS.)
